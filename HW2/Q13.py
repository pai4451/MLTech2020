...
class FunctionalTiedAutoEncoder(nn.Module):
    def __init__(self, d_):
        ...
        torch.nn.init.uniform_(self.bias1.detach(), -
                               U(fan_in0, fan_out1), U(fan_in0, fan_out1))
        torch.nn.init.uniform_(self.bias1.detach(), -
                               U(fan_in1, fan_out2), U(fan_in1, fan_out2))
...
if __name__ == '__main__':
    ...
    plt.ylabel("$Ein(g)$")
    plt.title("Ein with contraint $w_{ij}^{(1)} = w_{ji}^{(2)}$")
